            ...
            <div class="col-lg-12 mb-1">
                <div class="card">
                    ...
                    <div class="card-body">
                                    ...
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-9">
                                            ...
                                            <div style="display: none" class="text-danger nameExists">
                                                <i>Category Name </i>Already Exists
                                            </div>
                                        </div>
                        ...
                        <form style="display: none;" id="editCategory" action="{{ route('category.update') }}"
...
@push('script')
    ...
    <script>
        $('input[name="name"]').on('keyup', function() {
            ...
            checkCategoryName(cateId, input);
        });

        function checkCategoryName(id, name) {
            $.ajax({
        ...
        $('input[name="priority"]').on('keyup', function() {
            let input = $(this).val();
            let cateId = $('input[name="id"]').val();
            checkCategoryPriority(cateId, input);
        });

        function checkCategoryPriority(id, priority) {
            $.ajax({
                url: "{{ route('category.get') }}",
                type: "GET",
                data: {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response) {
                        ...
                        $('#Categorytitle').text('Update');
                        $('#editCategory').show();
                        $('#createCategory').hide();
                        $('#editCategory input[name="id"]').val(response[0].id);
                        $('#editCategory input[name="name"]').val(response[0].name);
                        $('#editCategory input[name="priority"]').val(response[0].priority);
                        $('#editCategory #IE').attr('src', imagePath);
