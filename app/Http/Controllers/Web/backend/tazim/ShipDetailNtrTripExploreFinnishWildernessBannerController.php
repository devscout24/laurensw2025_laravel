...
class ShipDetailNtrTripExploreFinnishWildernessBannerController extends Controller
{
    public function create()
    {
        $data = ShipDetailNtrTripExploreFinnishWildernessBanner::whereId(1)->first();
        return view('backend.layout.tazim.shipDetNatureExplrFinWild.create', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = ShipDetailNtrTripExploreFinnishWildernessBanner::find(1);

            if (! $data) {
                $data     = new ShipDetailNtrTripExploreFinnishWildernessBanner();
                $data->id = 1;
            }
            ...
            return redirect()->route('shipDetailsBan.create')->with('success', 'Created Successfully');
        } catch (Exception $e) {
